Deno.serve(async (req) => {
    ...
    try {
        ...
        const calculatedAmount = cartItems.reduce((sum: number, item: any) => sum + (item.price * item.quantity), 0);
        ...
        stripeParams.append('metadata[total_items]', cartItems.reduce((sum: number, item: any) => sum + item.quantity, 0).toString());
